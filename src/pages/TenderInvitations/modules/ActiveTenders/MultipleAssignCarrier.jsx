...
const MultipleAssignCarrier = ({ isOpen, onClose, selectedTrips = new Set() }) => {
  ...
  const onSubmit = (data) => {
    ...
    // Create an array of promises for all assignment operations
    const assignmentPromises = Array.from(selectedTrips).map((tripGuid) => {
      const computedData = {
        data: {
          guid: tripGuid,
      ...
      return tripsService.assignDriver(computedData);
    });
  ...
  return (
    ...
    >
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Assign Carrier to Multiple Trips</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <Text
            fontSize="14px"
            ...
            mb={4}
          >
            You are about to assign a carrier to <strong>{tripCount}</strong>{" "}
...
export default MultipleAssignCarrier;
