...
const httpRequestNoToken = axios.create({
  baseURL,
  timeout: 100000,
});
...
httpRequestNoToken.interceptors.request.use(
  (config) => {
...
httpRequestNoToken.interceptors.response.use(
  (response) => response.data.data || response.data,
  errorHandler
);

export default httpRequestNoToken;
